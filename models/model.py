...
class FineModel(nn.Module):
    def __init__(self, c_base, att_checkpoint, cls_checkpoint, max_factor=0.5, num_classes=200):
        ...
        model = CoarseModel_2(c_base, num_classes=num_classes)
        ...
        mod = torch.load(att_checkpoint, map_location='cpu')['state_dict']

        mod = replace_layer(mod, 'module')
        # for i in mod:
        #     print("att:", i)
        model = model.module
    ...
    def forward(self, x, inference=False):
        ...
        bx = self.vgg(bx)
        _, p_label = bx.topk(25, 1, True, True)

        # x_saliency = torch.zeros(p_label.size(0), 1, 56, 56).cuda()
        # for i in range(p_label.size(0)):
        #     x_saliency[i][0] = sx[i][p_label[i]].mean(0)
        # self.x_saliency = self.normalize_atten_maps(self.x_saliency)
...
def replace_layer(state_dict, keyword):
    keys = [key for key in state_dict.keys()]
    for key in keys:
        print("key:",key)
        if keyword in key:
