...
class Evaluator(nn.Module):
    def __init__(self, num_classes=200):
        ...
        self.classifier = make_classifier(512,num_classes)
    ...
    def forward(self, x):
        ...
        x =  self.model(x)
        self.feature_map = self.classifier(x)
        self.score = self.avgpool(self.feature_map).view(x.size(0),-1)
        return {'score': self.score,'fea':self.feature_map}

    def get_fused_cam(self):
        _, target = self.score.topk(1, 1, True, True)
        cam = self.feature_map.detach()[range(self.score.size(0)), target.data,:,:]
        cam = F.interpolate(cam, size=(224, 224), mode='bilinear', align_corners=False)
        cam = cam.mean(1).unsqueeze(1)
        return cam
...
if __name__ == '__main__':
    ...
    fea= out['fea']
    _, target = score.topk(1, 1, True, True)
    cam = fea.detach()[range(score.size(0)), target.data, :, :]
    cam = F.interpolate(cam, size=(224, 224), mode='bilinear', align_corners=False)
    cam = cam.mean(1).unsqueeze(1)
    print(cam.shape)