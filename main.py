...
# 训练流程开始
def train_cls(train_loader, model, optimizer, epoch, criterion, args):
...
def train(train_loader, model, optimizer, epoch, criterion, args):
    ...
    for i, (images, mage_id, image_class, image_name, target) in enumerate(train_t):
        ...
        target_layer = model.module.vgg.model.features[23]
        ...
        for i in range(gradients.size(0)):
            mask_ = weight[i].squeeze(0).cpu().numpy()
            in_put = image_[i]
            # heatmap = intensity_to_rgb(mask_, normalize=True).astype('uint8')
            heatmap_BGR = cv2.cvtColor(mask_, cv2.COLOR_RGB2BGR)
            #
        ...
        if args.training_cls:
            ...
            loss = loss_eval + loss_entropy +0.02*loss_ac
    ...
    return top1.avg, losses.avg
# def evaluate(val_loader, model, epoch, criterion, args):
#     losses = AverageMeter('Loss')
...
#             loss = criterion[0](output, target)  # for evaluation
#             acc1, acc5 = accuracy(output, target, topk=(1, 5))
#             losses.update(loss.data.item(), images.size(0))
...
#
#     return top1_cls.avg, losses.avg
